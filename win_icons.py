...
.html) y lo devuelve como una imagen PIL, lista para convertir a
QPixmap en la interfaz (ver `pil_to_pixmap` en main.py).
...
muchos problemas tanto con pip como al empaquetar con PyInstaller.

Todo este módulo es "best effort": si falta Pillow, o no estamos en
Windows, o cualquier llamada falla, las funciones devuelven None, para
que quien las use recurra a un icono de repuesto (emoji) sin que la app
se rompa.
"""
...
# muchas tarjetas, sobre todo porque la caché en memoria (`_icon_cache`,
# más abajo) se vacía cada vez que se cierra la app. Guardar el resultado
# como PNG hace que la *primera* carga tras abrir la app también sea rápida.
...
# Caché de iconos ya extraídos (imagen PIL), para no releer el disco ni
...
def _disk_cache_key(path: str, size: int) -> str:
    ...
    # mismo nombre, el icono en caché quede obsoleto automáticamente en
    # vez de mostrar el antiguo.
    try:
...
def get_icon_image(path: str, size: int = 32):
    """Devuelve una imagen PIL (RGBA) con el icono de `path`, o None si no
    se puede.
    ...
    no es Windows, o falla la llamada), devuelve None para que quien
    ...
    try:
        ...
        if image is not None and not came_from_disk:
...
def compose_folder_icon(preview_paths: "list[str | None]", size: int = 40):
    """Miniatura de una carpeta interna, compuesta con los iconos de hasta
    3 de sus elementos (como hace el Explorador de Windows con las
    carpetas normales), en un mosaico 2x2. Si no hay ningún icono
    disponible para ninguno de los `preview_paths`, devuelve None (y quien
    llama debe usar el emoji de carpeta de repuesto)."""
    if Image is None:
        return None

    paths = [p for p in (preview_paths or []) if p][:3]
    if not paths:
        return None

    mini_size = max(8, size // 2 - 1)
    thumbs = []
    for p in paths:
        img = get_icon_image(p, mini_size)
        if img is not None:
            thumbs.append(img.resize((mini_size, mini_size), Image.LANCZOS))
    if not thumbs:
        return None

    canvas = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    positions = [(0, 0), (size - mini_size, 0), (size // 4, size - mini_size)]
    for img, (x, y) in zip(thumbs, positions):
...
def clear_disk_cache() -> None:
    ...
    try:
        if _DISK_CACHE_DIR.exists():
            for cache_file in _DISK_CACHE_DIR.glob("*.png"):
                try:
                    cache_file.unlink()
                except OSError:
                    pass
